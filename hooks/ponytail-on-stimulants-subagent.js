#!/usr/bin/env node
// Ponytail on Stimulants — Claude Code/Qoder subagent instruction hook.
// SessionStart context is parent-thread only, so active fork instructions are
// injected explicitly into each supported subagent.
//
// Scoping (opt-in, issue #506): set PONYTAIL_ON_STIMULANTS_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./ponytail-on-stimulants-runtime');

const mode = readMode();

// Absent flag or off means this fork is inactive; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
let matcherRe = null;
try {
  if (process.env.PONYTAIL_ON_STIMULANTS_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.PONYTAIL_ON_STIMULANTS_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
