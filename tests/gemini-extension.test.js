...
// always-on context, commands/*.toml for /ponytail-on-stimulants + /ponytail-on-stimulants-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring ponytail-on-stimulants.
...
const EXTENSION_NAME = 'ponytail-on-stimulants';
...
const REUSED_COMMANDS = ['commands/ponytail-on-stimulants.toml', 'commands/ponytail-on-stimulants-review.toml'];
const REUSED_SKILLS = ['skills/ponytail-on-stimulants/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. ponytail-on-stimulants's
...
test('manifest names the ponytail-on-stimulants extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the ponytail-on-stimulants rules', () => {
  const manifest = loadManifest();
  assert.ok(manifest.contextFileName, 'contextFileName must be set so rules load every session');
  const context = read(manifest.contextFileName).toLowerCase();
  for (const phrase of RULE_INVARIANTS) {
    assert.ok(context.includes(phrase.toLowerCase()), `context file missing rule invariant: "${phrase}"`);
  }
});
