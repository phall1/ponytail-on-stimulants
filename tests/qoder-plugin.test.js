...
test('qoder hooks config exists and registers UserPromptSubmit', () => {
  ...
  assert.ok(cmd.includes('ponytail-on-stimulants-mode-tracker.js'), 'must point at ponytail-on-stimulants-mode-tracker.js');
});

test('qoder rules file exists and is non-empty', () => {
  const rulesPath = path.join(root, '.qoder', 'rules', 'ponytail-on-stimulants.md');
  assert.ok(fs.existsSync(rulesPath), '.qoder/rules/ponytail-on-stimulants.md must exist');
  const content = fs.readFileSync(rulesPath, 'utf8').trim();
  assert.ok(content.length > 0, '.qoder/rules/ponytail-on-stimulants.md must not be empty');
  assert.ok(content.includes('Minimal architecture. Maximal execution.'), 'rules must contain the Ponytail on Stimulants identity');
});
...
test('qoder rules match AGENTS.md canonical body', () => {
  ...
  const qoderCopy = fs.readFileSync(path.join(root, '.qoder', 'rules', 'ponytail-on-stimulants.md'), 'utf8')
    .replace(/\r\n/g, '\n').trim();
  assert.equal(qoderCopy, canonical, '.qoder/rules/ponytail-on-stimulants.md drifted from AGENTS.md');
});

test('qoder runtime detects QODER_SESSION_ID and writes hookSpecificOutput JSON', () => {
  const { isQoder } = require('../hooks/ponytail-on-stimulants-runtime');
