#!/usr/bin/env python3
"""Agentic, multi-file benchmark for ponytail-on-stimulants.
...
ponytail-on-stimulants: the claude CLI is the harness (already installed, we run inside it). No SDK
...
ARMS = {
    ...
    "ponytail-on-stimulants":       lambda: _skill("skills/ponytail-on-stimulants/SKILL.md"),
...
PLUGIN_ARMS = ("ponytail-on-stimulants", "caveman")          # arms activated via --plugin-dir (vs raw --append prompts)
PLUGIN_CACHE = Path.home() / ".claude" / "plugins" / "cache"

def _plugin_env(name):
    return re.sub(r"[^A-Z0-9]+", "_", name.upper()) + "_PLUGIN_DIR"


def _plugin_dir(name):
    """Resolve a plugin cache dir from a portable override or the host cache."""
    env_key = _plugin_env(name)
    env = os.environ.get(env_key)
    if env: return env
    base = PLUGIN_CACHE / name / name
    versions = sorted(p for p in base.glob("*") if p.is_dir()) if base.exists() else []
    if not versions:
        sys.exit(f"{name} plugin dir not found under {base}; install the plugin or set {env_key}")
...
# tests is still explicitly allowed, so ponytail-on-stimulants's "leave a runnable check" discipline is not suppressed.
...
def _selfcheck_split(p: Path):
    ...
    check is the runnable check ponytail-on-stimulants's rule asks for -- a positive signal, not source bloat --
...
def code_stats(workdir: Path, selfcheck_as_test: bool = False):
    ...
    reclassified from source to test, so following ponytail-on-stimulants's 'leave a runnable check' rule is not
...
def _selftest_plugin_dir():
    ...
    fails = 0
    sentinel = "/tmp/ponytail-on-stimulants-selftest-plugin-dir"
    os.environ["PONYTAIL_ON_STIMULANTS_PLUGIN_DIR"] = sentinel
    try:
        ok_env = _plugin_dir("ponytail-on-stimulants") == sentinel
    finally:
        del os.environ["PONYTAIL_ON_STIMULANTS_PLUGIN_DIR"]
    print(f"{'ok ' if ok_env else 'XX '} plugin_dir   env  override honored")
    fails += 0 if ok_env else 1
    missing = "ponytail-on-stimulants-does-not-exist-xyz"          # no env, no cache entry -> must sys.exit
    try:
